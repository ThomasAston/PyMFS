...
def boundary_intpoints(I,J, surface, input_data):
    ...
    '''
    int_degree=8 # Choose degree of integration here
    r = input_data.radius
    ...
    if surface[1][1]-surface[0][1] == 0:
        # t1 = max(min(np.array(surface)[:,0])+0.0000001, node_x-r, contrib_x-r)
        t1=max(min(np.array(surface)[:,0])+0.0000001, node_x-r, contrib_x-r)
        # t2 = min(max(np.array(surface)[:,0]), node_x+r, contrib_x+r)
        t2=min(max(np.array(surface)[:,0])-0.0000001, node_x+r, contrib_x+r)
        ...
        # t1 = max(min(np.array(surface)[:,1]), node_y-r, contrib_y-r)
        # t2 = min(max(np.array(surface)[:,1]), node_y+r, contrib_y+r)
        t1=max(min(np.array(surface)[:,1])+0.0000001, node_y-r, contrib_y-r)
        t2=min(max(np.array(surface)[:,1])-0.0000001, node_y+r, contrib_y+r)
        if math.hypot(node_x-contrib_x, node_y-contrib_y)<2*r:
            for a in range(int_degree):
