...
for I in range(Ntot):
    for m in range(order):
        ...
        if coor[I,0] - r < -lx/2: # sphere intersects Neumann boundary
            ...
            x_int = boundary_integration_points(I,I,int_degree=6)
            integrand = np.zeros([len(x_int),1])
            for counter in range(len(x_int)):  
                point = (-lx/2,x_int[counter])
                h_I = shape_functions(I,point)
                
                f_s = -14*np.cos(np.pi*point[1])
                
                integrand[counter] = f_s*h_I[m]
            
            fHat_Im[I,m] = np.trapz(integrand[:,0],x=x_int)
            ...
            for counter in range(len(x_int)):
                ...
                integrand[counter] = u_s*dhI_dx[m]
            ...
            for J in range(Ntot):
                ...
                for n in range(order):
                    for counter in range(len(x_int)):
                        ...
                        integrand[counter] = dhImhJn_dx
                    
                    KU_ImJn[I][J][m][n] = np.trapz(integrand[:,0],x=x_int)
...
for a in range(NX):
    for b in range(NY):
        ...
        if Y[b] == 0:
            u_y0.append((X[a],u[b,a]))
        elif Y[b] ==1:
            u_y1.append((X[a],u[b,a]))
        
        # analytical = u_analytical(X[a],Y[b])
        u_save.append((X[a],Y[b],u[b,a]))
        error.append((X[a],Y[b],u_analytical(X[a],Y[b])-u[b,a]))


# np.savetxt("u_2d_6_y0.txt", u_y0)
# np.savetxt("u_2d_6_y1.txt", u_y1)
# np.savetxt("u_2d_6.txt", u_save)
np.savetxt("error_2d_3.txt", error)
