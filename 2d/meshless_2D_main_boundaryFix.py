...
def integration_points(I,J,int_degree):
    ...
    # Distance between nodes http://paulbourke.net/geometry/circlesphere/:
    d = math.hypot(node_x-contrib_x, node_y-contrib_y)
...
def boundary_integration_points(I,J,int_degree=6):
    node_x = coor[I,0]
    node_y = coor[I,1]
    ...
    if math.hypot(node_x-contrib_x, node_y-contrib_y) < 2*r:
        for a in range(int_degree):
            gauss_points.append(y1 + (y2-y1)/2 + x_int[a]*(y2-y1)/2)
            gauss_weights.append(w_int[a])
    
    gauss_points = np.array(gauss_points)
    gauss_weights = np.array(gauss_weights)
    

    return gauss_points,gauss_weights,y1,y2
...
for I in range(Ntot):
    for m in range(order):
        ...
        for counter in range(len(gauss_points)):
            ...
            f_Im[I][m] += (dy/2)*(dx/2)*wj*wi*h_I[m]*f_xy
        ...
        if coor[I,0] - r < -lx/2: # sphere intersects Neumann boundary
            ...
            for counter in range(len(gauss_points)):  
                ...
                h_I = shape_functions(I,point)
                wj = gauss_weights[counter]
                f_s = -14*np.cos(np.pi*point[1])
                
                integrand = f_s*h_I[m]
                fHat_Im[I,m] += ((y2-y1)/2)*wj*integrand
            ...
            for counter in range(len(gauss_points)):
                ...
                dhI_dx = (hI_xplus-h_I)/delta
                u_s = 8*np.cos(np.pi*gauss_points[counter])
                wj = gauss_weights[counter]
                integrand = u_s*dhI_dx[m]

                fHat_Im[I][m] -= ((y2-y1)/2)*wj*integrand
                # fHat_Im[I][m] -= wj*integrand
            
            for J in range(Ntot):
                # if coor[J,0] + r > lx/2:
                # if I==J:
                gauss_points,gauss_weights,y1,y2 = boundary_integration_points(I,J,int_degree=6)
                if gauss_points.size==0:
                    KU_ImJn[I][J][m][n] = 0
                else:
                    for n in range(order):
                        for counter in range(len(gauss_points)):
                            ...
                            delta = 0.00000001
                            xplus = point[0]+delta
                            xminus = point[0]-delta

                            point_xplus = (xplus,point[1])
                            point_xminus = (xminus,point[1])

                            hI_xminus = shape_functions(I,point_xminus)
                            hI_xplus = shape_functions(I,point_xplus)
                            dhI_dx = (hI_xplus-h_I)/delta
                            hJ_xminus = shape_functions(J,point_xminus)
                            hJ_xplus = shape_functions(J,point_xplus)
                            
                            dhImhJn_dx = (hI_xplus[m]*hJ_xplus[n] - hI_xminus[m]*hJ_xminus[n])/(2*delta) 
                            
                            KU_ImJn[I][J][m][n] += ((y2-y1)/2)*wj*dhImhJn_dx
                            # KU_ImJn[I][J][m][n] += wj*dhImhJn_dx
...
# gauss_plot, weights,x1,x2,y1,y2 = integration_points(8,8,int_degree=4)
# # print(weights)
# ax10 = plt.axes()
